...
import { FeatureRequestError, type FeatureRequestActor } from '@/lib/services/feature-requests'
...
export async function getFeatureRequestActor(request: NextRequest): Promise<FeatureRequestActor> {
  const session = await auth.api.getSession({ headers: request.headers })
  if (!session?.user) throw new FeatureRequestError('UNAUTHORIZED', 'Meld asseblief aan')
...
export function requireJsonMutation(request: NextRequest) {
  if (!request.headers.get('content-type')?.toLowerCase().startsWith('application/json')) {
    throw new FeatureRequestError('INVALID', 'Gebruik asseblief JSON')
  }
  const origin = request.headers.get('origin')
  if (origin && origin !== request.nextUrl.origin) {
    throw new FeatureRequestError('FORBIDDEN', 'Ongeldige versoek-oorsprong')
  }
}

export function featureRequestErrorResponse(error: unknown, operation: string) {
  ...
  console.error(`Feature request API failed: ${operation}`)
